## Git rules

- [Branch name](https://eaton-corp.atlassian.net/wiki/spaces/CYME/pages/133238451/GitHub+Tutorial+-+Fixing+a+simple+bug+in+C+code)
  - `wi/main/<<JIRA-TICKET>>_description_max_40_chars`
- Pull request
  - [Git convention](https://eaton-corp.atlassian.net/wiki/spaces/CYME/pages/115218008/Git+Conventions)
    - **fix** for a bug fix
    - **feat** for implementation of a feature
    - **docs** for documentation
    - **refactor** for a refactoring
    - **chore** for a general maintenance task that doesn’t affect the code
    - Examples:
      - feat(CQAM-1234): support for super new technology
      - fix(CQAM-1235): big crash removed from feature ABC
      - docs(CQAM-1237): readme updated
  - [Commit message convention](https://eaton-corp.atlassian.net/wiki/spaces/CYME/pages/133439834/CYME+-+S+T+-+Commit+Message+Convention)
  - [Other git branch naming rules](https://eaton-corp.atlassian.net/wiki/spaces/REMON/pages/1140228792/GitHub+Branch+Naming+Rules)
## Links

- [Jira](https://eaton-corp.atlassian.net/jira)
- [Confluence](https://eaton-corp.atlassian.net/wiki/home)

- [Git](https://github.com/etn-utilities)
- [DDP](https://github.com/etn-utilities/cyme-di-cust-lucelec)
- [FME Plugin](https://github.com/etn-utilities/cyme-di-plugins)
- [Gateway](https://github.com/etn-utilities/cyme-di-gateway-template)
## Helpful pages to working on CYMDist Plugin

- [CYMDist Plugin](https://eaton-corp.atlassian.net/wiki/spaces/CYME/pages/653527076/CYMDist+Plugin)
## Tools

 How to check if a dll is x86 or x64

 - C:\dev\tools\Sigcheck\sigcheck.exe "C:\dev\git\cyme-di-plugins\FME Plugins\src\GATEWAY\dll\x86\Debug\CYMDistd.dll"
## Folders

Software's to install: Mapped folder `cyme-install` or `\\png2448-01.cyme.local`

user: cyme\gateway
pwd: gateway

## Servers

### C# libraries

- \\vm-cymbuilder.cyme.local\  (???)
## Work environment

- Remote desktop: 10.106.28.151
  user: cyme\gateway
  pwd: gateway

- vm-PluginFME.cyme.local -> To test the C++ plugin (CYMEdist)

**IMPORTANT:** To do not have problems to access the local resources, this credential must be added into Windows Credential

![Windows Credential](local-resources-windows-credentials.png)



